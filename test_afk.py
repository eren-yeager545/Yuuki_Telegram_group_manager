...
async def test_afk_cmd_group_with_reason_and_emojis(tmp_path, monkeypatch):
    ...
    afk_rec = store.get_afk(-10012345, 111)
...
async def test_afk_cmd_group_without_reason_uses_default(tmp_path, monkeypatch):
    ...
    afk_rec = store.get_afk(-10012345, 222)
    assert afk_rec is not None
...
async def test_afk_cmd_twice_updates_record(tmp_path, monkeypatch):
    ...
    store.set_afk(-100, 333, "reason 1", 1000)
    assert store.get_afk(-100, 333)[0] == "reason 1"
    ...
    afk_rec = store.get_afk(-100, 333)
    assert afk_rec[0] == "reason 2"
    assert afk_rec[1] > 1000


@pytest.mark.asyncio
async def test_afk_per_chat_isolation(tmp_path, monkeypatch):
    monkeypatch.setattr(store, 'DB_PATH', str(tmp_path / "test_afk.db"))
    store.init_db()

    store.set_afk(-100111, 444, "sleeping", int(time.time()))

    assert store.get_afk(-100111, 444) is not None
    assert store.get_afk(-100222, 444) is None
...
async def test_afk_mention_by_username_and_no_username(tmp_path, monkeypatch):
    ...
    store.set_afk(-100, 555, "away eating", int(time.time()) - 100)

    store.upsert_user(556, "NoUser", None)
    store.set_afk(-100, 556, "no username afk", int(time.time()) - 200)
...
async def test_afk_multiple_mentions(tmp_path, monkeypatch):
    ...
    store.set_afk(-100, 101, "reason 1", int(time.time()) - 10)
    store.set_afk(-100, 102, "reason 2", int(time.time()) - 20)
...
async def test_afk_mention_by_reply(tmp_path, monkeypatch):
    ...
    store.set_afk(-100, 666, "busy working", int(time.time()) - 300)
...
async def test_message_router_afk_return_and_subsequent_messages(tmp_path, monkeypatch):
    ...
    store.set_afk(chat_id, user_id, "out door", int(time.time()) - 60)
    ...
    assert store.get_afk(chat_id, user_id) is not None

    # 2. Sending normal message SHOULD return user and clear AFK
    msg2 = AsyncMock()
    update2 = MagicMock()
    update2.effective_message = msg2
    update2.effective_chat.id = chat_id
    update2.effective_chat.type = "supergroup"
    update2.effective_chat.title = "Test Supergroup"
    ...
    assert store.get_afk(chat_id, user_id) is None
...
async def test_datadel_clears_afk(tmp_path, monkeypatch):
    ...
    store.set_afk(-100, 888, "sleeping", int(time.time()))
    assert store.get_afk(-100, 888) is not None

    store.delete_user_data(888)
    assert store.get_afk(-100, 888) is None
